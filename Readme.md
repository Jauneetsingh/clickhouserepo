...
**Subject: Re: ClickHouse 25.8 Deployment - Configuration Issues & Requirements**

Hello,

Thank you for reaching out. I've reviewed your case and have addressed all three requirements. Here is a summary of my findings and recommendations:

### 1. Limiting Databases to 3

To enforce a hard limit on the number of databases, use the `max_database_num_to_throw` server setting in your ClickHouse server configuration:
...
Place this in a file under `/etc/clickhouse-server/config.d/` (e.g., `custom.xml`). Note that system databases (`system`, `INFORMATION_SCHEMA`, `information_schema`) are not counted towards this limit, but the `default` database is. So with a limit of 3, you can create 2 additional databases beyond `default`.

When the limit is exceeded, ClickHouse will return error code 725 (`TOO_MANY_DATABASES`).

### 2. User Configuration

I've configured both users as requested:

**admin**: Uses the `default` profile with no restrictions and has `access_management` enabled, giving the same permission level as the `default` user.

**developer**: Has a custom profile with:
- `max_execution_time`: 0.5 seconds (500ms) — queries exceeding this are terminated with error 159 (`TIMEOUT_EXCEEDED`).
- `max_memory_usage`: 104857600 bytes (100MB) — queries exceeding this are terminated with error 241 (`MEMORY_LIMIT_EXCEEDED`).

For the row-level security on `system.tables`, I've created row policies using SQL:
- The developer user can only see rows where `database != 'system'`.
- **Important**: When a row policy is defined on any table, it affects ALL users. You must create an explicit permissive policy (`USING 1`) for admin/default users, otherwise they will also lose visibility.

### 3. Keeper Configuration

I found **two issues** in your Keeper configuration:

**Issue 1 (Critical): Malformed XML Tags**
All XML tag names containing underscores are broken across multiple lines. For example, `<keeper_server>` appears as `<keeper\nserver>`, `<coordination_settings>` as `<coordination\n_\nsettings>`, etc. This makes the XML completely unparseable, causing a `SAXParseException` at startup. Every compound tag name needs to be on a single line with proper underscores.

**Issue 2: `listen_host` Placement**
The `<listen_host>0.0.0.0</listen_host>` setting is placed inside the `<keeper_server>` block. For ClickHouse Keeper, this setting must be at the root `<clickhouse>` level. Without this fix, Keeper only binds to `127.0.0.1` and is unreachable from other containers/servers.

I've attached the corrected configuration file for your reference.

**Regarding ClickHouse Keeper**: It is a coordination service (replacement for Apache ZooKeeper) required for replicated tables and distributed operations. For production deployments, key best practices include:
- Deploy an **odd number of nodes** (3 or 5) for fault tolerance.
- Use **dedicated servers** separate from ClickHouse to avoid resource contention.
- Use **fast SSD storage** for log and snapshot paths.
- Ensure **low-latency networking** between Keeper nodes.
- Monitor health via `system.zookeeper_connection` or four-letter commands.

**A note on versioning**: The Docker tag `25.8.1` does not exist. The ClickHouse 25.8 LTS branch uses tags like `25.8` (latest), `25.8.16`, or the full build number `25.8.16.34`. I recommend using the `25.8` tag.

Please let me know if you have any questions or need further assistance.

Best regards,  
Support Engineer, ClickHouse
